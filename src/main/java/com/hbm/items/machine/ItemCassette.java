...
public class ItemCassette extends Item implements IDynamicModels {
    ...
    public IItemColor getItemColorHandler() {
        return (stack, tintIndex) -> {
            if (tintIndex == 1) {
                int color = ItemCassette.TrackType.byIndex(stack.getItemDamage()).getColor();
                if (color < 0) {
                    color = 0xFFFFFF;
                }
                return color;
    ...
    public void registerModel() {
        for (TrackType track : TrackType.VALUES.values()) {
            ModelLoader.setCustomModelResourceLocation(this, track.getId(), new ModelResourceLocation(this.getRegistryName(), "inventory"));
        }
    }
    ...
    public static class TrackType {
        ...
        private TrackType(String name, SoundEvent loc, SoundType sound, int color, int volume, int id) {
            ...
            if (VALUES.containsKey(id)) {
                MainRegistry.logger.error("ID collision when registering siren tracks! (id: {}, old track: \"{}\", new track: \"{}\")", id, VALUES.get(id).title, name);
            }
            VALUES.put(id, this);
        }
